...
public abstract class TaskScheduler {
    ...
    private final BlockingQueue<TaskDescription> fakeRunnableTaskQueue =
            new LinkedBlockingQueue<TaskDescription>();
    ...
    TaskDescription getNextTask() {
        ...
        try {
            if (!runnableTaskQueue.isEmpty()) {
                task = runnableTaskQueue.take();
            } else if (runnableTaskQueue.isEmpty() && !fakeRunnableTaskQueue.isEmpty()) {
                task = fakeRunnableTaskQueue.take();
            }
     ...
     * Returns the current number of runnable tasks (for testing).
     */
    int fakeRunnableTasks() {
        return fakeRunnableTaskQueue.size();
    }

    //TODO Not sure if we need this
    synchronized void fakeTasksFinished(List<TFullTaskId> finishedTasks, boolean isFake) {
